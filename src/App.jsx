...
// ─── ENV — .env.local (Vite) with localStorage fallback ───────────────────
// Priority: .env.local vars → localStorage (UI input) → ""
// In deployment: set VITE_GITHUB_TOKEN etc in Vercel env vars
// In dev: create .env.local with the vars below
const ENV_KEYS = {
  GITHUB_TOKEN: "kyla_github_token",
  LLM_KEY:      "kyla_llm_key",
  LLM_MODEL:    "kyla_llm_model",
};
const VITE_ENV = {
  GITHUB_TOKEN: typeof import.meta !== "undefined" ? (import.meta.env?.VITE_GITHUB_TOKEN || "") : "",
  LLM_KEY:      typeof import.meta !== "undefined" ? (import.meta.env?.VITE_LLM_KEY      || "") : "",
  LLM_MODEL:    typeof import.meta !== "undefined" ? (import.meta.env?.VITE_LLM_MODEL    || "") : "",
};
const Env = {
  get:      (k)   => VITE_ENV[k] || (()=>{ try { return localStorage.getItem(ENV_KEYS[k]) || ""; } catch { return ""; } })(),
  set:      (k,v) => { try { if (!VITE_ENV[k]) localStorage.setItem(ENV_KEYS[k], v); } catch {} },
  clear:    (k)   => { try { localStorage.removeItem(ENV_KEYS[k]); } catch {} },
  clearAll: ()    => { Object.values(ENV_KEYS).forEach(k => { try { localStorage.removeItem(k); } catch {} }); },
};
...
function HackerAgentTab() {
  ...
  const [llmKey,    setLlmKey]    = useState(() => Env.get("LLM_KEY"));
  const [llmModel,  setLlmModel]  = useState(() => Env.get("LLM_MODEL") || "bai");
  ...
  // Security headers via Mozilla Observatory (100% free, no key)
  // Falls back to extracting headers from allorigins response
  const getHeaders = async (url) => {
    // Method 1: Mozilla Observatory scan (free, no key, no rate limit)
    try {
      const domain = new URL(url).hostname;
      const scanRes = await fetch(
        `https://observatory-api.mdn.mozilla.net/api/v2/analyze?host=${encodeURIComponent(domain)}`,
        { method: "POST" }
      );
      if (scanRes.ok) {
        const scanData = await scanRes.json();
        // Poll for results (max 3 tries)
        for (let i = 0; i < 3; i++) {
          ...
          if (res2.ok) {
            ...
            if (d2.response_headers) {
              // Convert Observatory response_headers to raw header string format
              return Object.entries(d2.response_headers)
                .map(([k,v]) => `${k}: ${v}`)
                .join("\n");
            }
          }
    ...
    // Method 2: Extract from allorigins response object
    try {
      const proxied = `https://api.allorigins.win/get?url=${encodeURIComponent(url)}`;
      const r = await fetch(proxied);
      if (r.ok) {
        const w = await r.json();
        // allorigins returns content_type — build minimal header string
        const lines = [];
        if (w.status?.content_type) lines.push(`content-type: ${w.status.content_type}`);
        if (w.status?.http_code)    lines.push(`x-http-status: ${w.status.http_code}`);
        return lines.join("\n") || "";
      }
    } catch {}
  ...
  // DNS lookup via Cloudflare DoH (100% free, no key, unlimited)
  const getDNS = async (domain) => {
    try {
      const types = ["A", "MX", "NS", "CNAME"];
      const results = [];
      await Promise.all(types.map(async (t) => {
        ...
        (d.Answer || []).forEach(a => results.push(`${t}\t${a.data}`));
      }));
      return results.join("\n");
    } catch { return ""; }
  };
  ...
  const startScan = async () => {
    ...
    await addLog("fetch_headers via Mozilla Observatory + Cloudflare DoH (free)", "step");
    ...
    if (rawHeaders) {
      await addLog(`${headerList.length} response headers captured (Mozilla Observatory + Cloudflare DoH)`, "detail");
    } else {
      consLog.push({ level: "WRN", msg: "Header fetch incomplete — using page content analysis only" });
    }
...
REAL HTTP HEADERS:
${headerStr || "(headers via page analysis)"}
...
[{"id":"AI-001","title":"Finding Title","severity":"critical|high|medium|low|info","endpoint":"${url}","method":"GET","evidence":"Specific evidence based on the real data above","cwe":"CWE-XXX","wave":5}]`;
    ...
    try {
      // LLM synthesis — B.ai / OpenAI / Claude
      let aiTxt = "[]";
      const isOAIcompat = llmModel === "bai" || llmModel === "openai";

      if (isOAIcompat && llmKey) {
        // OpenAI-compatible path (B.ai uses same format)
        const endpoint = llmModel === "bai"
          ? "https://api.b.ai/v1/chat/completions"
          : "https://api.openai.com/v1/chat/completions";
        const model = llmModel === "bai" ? "gpt-5.2" : "gpt-4o-mini";
        const oRes = await fetch(endpoint, {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${llmKey}` },
          body: JSON.stringify({ model, max_tokens: 800, messages: [{ role: "user", content: aiPrompt }] })
        });
        const oData = await oRes.json();
        aiTxt = oData.choices?.[0]?.message?.content || "[]";
        setCost(prev => prev + (oData.usage?.total_tokens || 500) * 0.000001);
      } else {
        // Anthropic — use injected key or artifact auto-proxy
        const headers = { "Content-Type": "application/json" };
        if (llmKey && llmModel === "claude") headers["x-api-key"] = llmKey;
        const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
          method: "POST", headers,
          body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 800, messages: [{ role: "user", content: aiPrompt }] })
        });
        const aiData = await aiRes.json();
        aiTxt = aiData.content?.[0]?.text || "[]";
      }

      const aiFnds = JSON.parse(aiTxt.replace(/```json\n?|```\n?/g, "").trim());
      if (Array.isArray(aiFnds)) allFindings.push(...aiFnds);
      const modelLabel = llmModel === "bai" ? "B.ai gpt-5.2" : llmModel === "openai" ? "GPT-4o-mini" : "Claude";
      await addLog(`${modelLabel} added ${aiFnds.length} finding(s)`, "detail");
    } catch (e) {
      await addLog(`LLM synthesis skipped: ${e.message}`, "detail");
    }
  ...
  return (
    <div style={{ display: "flex", flex: 1, overflow: "hidden", background: "#0a0e14", color: "#e2e8f0", fontFamily: "'SF Mono','Fira Mono',monospace", fontSize: 12 }}>
      <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
        ...
        {/* ── Scan bar ── */}
        <div style={{ borderBottom: "1px solid #1e2938", background: "#070b10", flexShrink: 0 }}>
          {/* Main row */}
          <div style={{ padding: "10px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button onClick={() => setShowKeys(!showKeys)}
              style={{ background: showKeys ? "#1e2938" : "transparent", color: htKey ? "#22c55e" : "#6b7280", border: "1px solid #1e2938", padding: "8px 10px", fontSize: 9, cursor: "pointer", fontFamily: "inherit", borderRadius: 2, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ fontSize: 11 }}>⚿</span> API KEYS
            </button>
            ...
            <div style={{ fontSize: 10, color: "#6b7280", marginLeft: "auto", display: "flex", gap: 12, alignItems: "center" }}>
              ...
              {cost > 0 && <span style={{ color: "#22c55e" }}>${cost.toFixed(2)}</span>}
              <span style={{ color: "#1e2938", fontSize: 9, letterSpacing: "0.06em" }}>REAL HTTP PROBING</span>
            </div>
          </div>

          {/* API keys panel */}
          {showKeys && (
            <div style={{ padding: "10px 16px 12px", borderTop: "1px solid #1e2938", display: "flex", alignItems: "flex-start", gap: 20 }}>
              {/* Free APIs info */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, letterSpacing: "0.12em", fontWeight: 700, color: "#4b5563", marginBottom: 10 }}>
                  FREE RECON APIS — NO KEY NEEDED
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[
                    { name: "Mozilla Observatory", desc: "Security header analysis + scoring", url: "observatory.mozilla.org", free: true },
                    { name: "Cloudflare DoH",       desc: "DNS lookup (A, MX, NS, CNAME)",    url: "cloudflare-dns.com",    free: true },
                    { name: "allorigins.win",        desc: "CORS proxy — page fetch",          url: "allorigins.win",        free: true },
                    { name: "corsproxy.io",          desc: "CORS fallback proxy",              url: "corsproxy.io",          free: true },
                  ].map(api => (
                    <div key={api.name} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", background: "#111827", border: "1px solid #1e2938", borderRadius: 2 }}>
                      <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#22c55e", flexShrink: 0 }} />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 9, color: "#9ca3af", fontWeight: 600 }}>{api.name}</div>
                        <div style={{ fontSize: 8, color: "#374151" }}>{api.desc}</div>
                      </div>
                      <span style={{ fontSize: 8, color: "#22c55e", letterSpacing: "0.04em" }}>FREE</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* LLM API key */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, letterSpacing: "0.12em", fontWeight: 700, color: "#4b5563", marginBottom: 5, display: "flex", alignItems: "center", gap: 8 }}>
                  LLM API KEY
                  <span style={{ color: "#374151", fontWeight: 400 }}>— AI synthesis wave, finding enrichment</span>
                  <span style={{ color: "#22c55e", fontSize: 8, marginLeft: "auto" }}>auto-injected in artifact if empty</span>
                </div>
                {/* Model toggle */}
                <div style={{ display: "flex", gap: 1, marginBottom: 6 }}>
                  {[
                    ...
                    <button key={v} onClick={() => { setLlmModel(v); Env.set("LLM_MODEL", v); }}
                      style={{ flex: 1, padding: "5px 0", border: "1px solid #1e2938", background: llmModel === v ? "#1e2938" : "transparent", color: llmModel === v ? "#22c55e" : "#4b5563", fontSize: 9, cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.04em" }}>
                      {l}
                    </button>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <input value={llmKey} onChange={e => { setLlmKey(e.target.value); Env.set("LLM_KEY", e.target.value); }} type="password"
                    placeholder={
                      llmModel === "bai"    ? "sk-z8d9w...qb63" :
                      llmModel === "claude" ? "sk-ant-api03-..." :
                                              "sk-..."
                    }
                    style={{ flex: 1, background: "#111827", border: `1px solid ${llmKey ? "#22c55e44" : "#1e2938"}`, padding: "6px 10px", fontSize: 11, fontFamily: "monospace", color: llmKey ? "#22c55e" : "#6b7280", outline: "none", borderRadius: 2 }} />
                  {llmKey && <button onClick={() => { setLlmKey(""); Env.clear("LLM_KEY"); }} style={{ background: "transparent", border: "1px solid #1e2938", color: "#6b7280", padding: "0 10px", fontSize: 10, cursor: "pointer", borderRadius: 2 }}>✕</button>}
                </div>
                {/* B.ai info box */}
                {llmModel === "bai" && (
                  <div style={{ marginTop: 6, padding: "7px 10px", background: "#111827", border: "1px solid #1e2938", borderRadius: 2 }}>
                    <div style={{ fontSize: 9, color: "#22c55e", letterSpacing: "0.06em", fontWeight: 700, marginBottom: 3 }}>B.AI · chat.b.ai/key</div>
                    <div style={{ fontSize: 8, color: "#6b7280", lineHeight: 1.6 }}>
                      Endpoint: api.b.ai/v1/chat/completions<br/>
                      Model: gpt-5.2 · OpenAI-compatible format<br/>
                      Paste your <span style={{ color: "#9ca3af" }}>sk-z8d9w…</span> key from chat.b.ai/key
                    </div>
                  </div>
                )}
                {llmModel !== "bai" && (
                  <div style={{ fontSize: 8, color: "#374151", marginTop: 4, lineHeight: 1.6 }}>
                    {llmModel === "claude"
                      ? "Optional — artifact auto-injects Claude key. Add your own for higher limits."
                      : "OpenAI key required. Get at platform.openai.com/api-keys"}
                    <div style={{fontSize:8,color:"#374151",marginTop:3}}>Saved to localStorage — persists across sessions · works like .env</div>
                  </div>
                )}
              </div>

              {/* Status indicators */}
              <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", gap: 5, paddingTop: 18 }}>
                {[
                  { label: "LLM",             ok: true, note: llmKey ? `${llmModel === "bai" ? "B.ai gpt-5.2" : llmModel === "openai" ? "OpenAI" : "Claude"} ✓` : "Claude auto-injected" },
                  { label: "Mozilla Obs.",    ok: true, note: "Header scan · free · no key" },
                  { label: "Cloudflare DoH",  ok: true, note: "DNS lookup · free · no key" },
                  { label: "allorigins.win",  ok: true, note: "CORS proxy · free" },
                ].map(s => (
                  <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: s.ok ? "#22c55e" : "#374151", flexShrink: 0 }} />
                    <span style={{ fontSize: 9, color: "#6b7280", minWidth: 100 }}>{s.label}</span>
                    <span style={{ fontSize: 9, color: s.ok ? "#22c55e" : "#374151" }}>{s.note}</span>
                  </div>
                ))}
              </div>
            </div>
...
function RepoScanTab() {
  ...
  const [token,   setToken]   = useState(() => Env.get("GITHUB_TOKEN"));
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden",background:"#f8f8f8",fontFamily:"'Helvetica Neue',Helvetica,sans-serif"}}>
      ...
      <div style={{width:300,borderRight:"1px solid #e0e0e0",display:"flex",flexDirection:"column",background:"#fff",flexShrink:0}}>
        ...
        <div style={{padding:"20px 18px",borderBottom:"1px solid #f0f0f0"}}>
          ...
          {/* GitHub Token — always visible */}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:8,letterSpacing:"0.12em",fontWeight:700,color:"#b0b0b0",marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span>GITHUB TOKEN</span>
              {token
                ? <span style={{color:"#16a34a",fontWeight:600}}>✓ GraphQL mode active</span>
                : <span style={{color:"#d97706"}}>REST mode (60 req/hr)</span>}
            </div>
            <div style={{display:"flex",gap:6}}>
              <input value={token} onChange={e=>{ setToken(e.target.value); Env.set("GITHUB_TOKEN", e.target.value); }}
                type="password" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx"
                style={{flex:1,border:`1.5px solid ${token?"#16a34a":"#e0e0e0"}`,padding:"8px 10px",fontSize:11,fontFamily:"monospace",color:token?"#16a34a":"#0a0a0a",outline:"none",background:"#fff"}}/>
              {token&&<button onClick={()=>{ setToken(""); Env.clear("GITHUB_TOKEN"); }} style={{border:"1px solid #e0e0e0",background:"#fff",cursor:"pointer",padding:"0 10px",fontSize:12,color:"#9b9b9b"}}>✕</button>}
            </div>
            <div style={{fontSize:9,color:"#9b9b9b",marginTop:5,lineHeight:1.7}}>
              Get at <span style={{fontFamily:"monospace",color:"#0a0a0a"}}>github.com/settings/tokens</span> → Generate new token (classic) → centang <span style={{fontFamily:"monospace",color:"#0a0a0a"}}>public_repo</span><br/>
              <span style={{color:token?"#16a34a":"#d97706"}}>{token?"✓ With token: GraphQL (1 call) + 5000 req/hr + file checks":"Without: 4 REST calls + 60 req/hr + no file checks"}</span>
              <br/><span style={{color:"#c0c0c0"}}>Saved to localStorage — works like .env · never leaves your browser</span>
            </div>
          </div>
...
function LandingPage({ onEnter }) {
  ...
  const features = [
    { id:"skills", icon:"◈", label:"Skills Marketplace", desc:"Browse 1000+ real Claude Code skills from Anthropic, Google, Stripe, HashiCorp, Trail of Bits and the community. Download SKILL.md in one click.", tag:"1000+ INDEXED", accent:"#0a0a0a" },
    { id:"hacker", icon:"Ha", label:"Hacker Agent", desc:"Real HTTP security scanner. Probes live targets for exposed files, security header misconfigs, secret leaks, CORS issues and more. Powered by HackerTarget API + Claude.", tag:"REAL PROBING", accent:"#22c55e" },
    { id:"reposcan", icon:"Rs", label:"RepoScan", desc:"Score any GitHub repo 0–100 across 6 trust dimensions from live API data. GraphQL mode (1 call) or REST fallback. Direct competitor to Zauth RepoScan.", tag:"LIVE GITHUB DATA", accent:"#2563eb" },
  ];

  return (
    <div style={{height:"100vh",display:"flex",flexDirection:"column",fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif",background:"#fff",color:"#0a0a0a",overflow:"hidden"}}>
      {/* Topbar */}
      <div style={{borderBottom:"1.5px solid #0a0a0a",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 40px",height:52,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:24,height:24,background:"#0a0a0a",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <span style={{color:"#fff",fontSize:11}}>◈</span>
          </div>
          <div style={{lineHeight:1}}>
            <div style={{fontSize:12,fontWeight:700,letterSpacing:"-0.01em"}}>Kyla Agent</div>
            <div style={{fontSize:9,color:"#9b9b9b",letterSpacing:"0.08em",marginTop:2}}>AGENT PLATFORM</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:"#22c55e"}}/>
          <span style={{fontSize:10,color:"#9b9b9b",letterSpacing:"0.06em"}}>LIVE</span>
        </div>
      </div>

      {/* Hero */}
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 40px",position:"relative",overflow:"hidden"}}>
        {/* Grid bg */}
        <div style={{position:"absolute",inset:0,opacity:0.035,backgroundImage:"linear-gradient(#0a0a0a 1px,transparent 1px),linear-gradient(90deg,#0a0a0a 1px,transparent 1px)",backgroundSize:"40px 40px",pointerEvents:"none"}}/>
        {/* Big number */}
        <div style={{position:"absolute",right:"4%",top:"50%",transform:"translateY(-50%)",fontSize:"clamp(140px,20vw,280px)",fontWeight:200,color:"#f2f2f2",lineHeight:1,letterSpacing:"-0.06em",userSelect:"none",pointerEvents:"none"}}>3</div>

        <div style={{maxWidth:800,width:"100%",position:"relative",zIndex:1}}>
          {/* Badge */}
          <div style={{display:"inline-flex",alignItems:"center",gap:8,border:"1px solid #e0e0e0",padding:"4px 14px",marginBottom:28}}>
            <span style={{width:5,height:5,borderRadius:"50%",background:"#22c55e"}}/>
            <span style={{fontSize:9,letterSpacing:"0.14em",fontWeight:700,color:"#9b9b9b"}}>KYLA AGENT — AI SECURITY INTELLIGENCE PLATFORM</span>
          </div>

          {/* H1 */}
          <h1 style={{fontSize:"clamp(40px,6.5vw,78px)",fontWeight:800,letterSpacing:"-0.035em",lineHeight:1.04,margin:"0 0 22px"}}>
            Skills. Scanner.<br/>Trust Scores.
          </h1>

          {/* Sub */}
          <p style={{fontSize:"clamp(14px,1.5vw,17px)",color:"#6b6b6b",lineHeight:1.8,maxWidth:500,margin:"0 0 40px",fontWeight:300}}>
            Three tools in one platform — install Claude AI skills, run real
            security scans on live targets, and score GitHub repos from actual API data.
          </p>

          {/* CTAs */}
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
            <button onClick={()=>onEnter("skills")}
              style={{background:"#0a0a0a",color:"#fff",border:"none",padding:"14px 32px",fontSize:13,fontWeight:700,cursor:"pointer",letterSpacing:"0.06em",fontFamily:"inherit"}}
              onMouseEnter={e=>e.currentTarget.style.opacity=".82"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
              ENTER PLATFORM →
            </button>
            <button onClick={()=>onEnter("hacker")}
              style={{background:"transparent",color:"#0a0a0a",border:"1.5px solid #0a0a0a",padding:"13px 24px",fontSize:13,fontWeight:500,cursor:"pointer",letterSpacing:"0.04em",fontFamily:"inherit"}}
              onMouseEnter={e=>e.currentTarget.style.background="#f5f5f5"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              Try Hacker Agent
            </button>
            <button onClick={()=>onEnter("reposcan")}
              style={{background:"transparent",color:"#6b6b6b",border:"1.5px solid #e0e0e0",padding:"13px 24px",fontSize:13,fontWeight:400,cursor:"pointer",letterSpacing:"0.04em",fontFamily:"inherit"}}
              onMouseEnter={e=>e.currentTarget.style.borderColor="#0a0a0a"} onMouseLeave={e=>e.currentTarget.style.borderColor="#e0e0e0"}>
              Try RepoScan
            </button>
          </div>
        </div>
      ...
      {/* Feature row */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",borderTop:"1.5px solid #0a0a0a",flexShrink:0}}>
        {features.map((f,i)=>{
          const isH = hovered===f.id;
          return (
            <div key={f.id} onClick={()=>onEnter(f.id)}
              onMouseEnter={()=>setHovered(f.id)} onMouseLeave={()=>setHovered(null)}
              style={{padding:"26px 30px",cursor:"pointer",borderRight:i<2?"1px solid #e0e0e0":"none",background:isH?"#0a0a0a":"#fff",transition:"background 0.15s"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                <div style={{width:32,height:32,background:isH?(f.accent==="#0a0a0a"?"#fff":f.accent):(f.accent==="#0a0a0a"?"#0a0a0a":"#f0f0f0"),display:"flex",alignItems:"center",justifyContent:"center",borderRadius:2,flexShrink:0}}>
                  <span style={{fontSize:10,fontWeight:800,letterSpacing:"-0.02em",color:isH?(f.accent==="#0a0a0a"?"#0a0a0a":"#fff"):(f.accent==="#0a0a0a"?"#fff":f.accent)}}>{f.icon}</span>
                </div>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:isH?"#fff":"#0a0a0a"}}>{f.label}</div>
                  <div style={{fontSize:8,letterSpacing:"0.1em",fontWeight:700,color:isH?"rgba(255,255,255,0.35)":"#b0b0b0",marginTop:2}}>{f.tag}</div>
                </div>
              </div>
              <p style={{fontSize:12,lineHeight:1.75,margin:"0 0 14px",color:isH?"rgba(255,255,255,0.55)":"#6b6b6b",fontWeight:300}}>{f.desc}</p>
              <div style={{fontSize:11,fontWeight:600,color:isH?"rgba(255,255,255,0.65)":"#0a0a0a",display:"flex",alignItems:"center",gap:6,letterSpacing:"0.04em"}}>
